...
After confirmation, define a schema matching the grade contract. A named class is a good default
when the grade is object-shaped:
...
different schemas. Later graders can read prior decoded results through `prevResults.<stageName>`.
...
The grade function returns the schema's encoded value (a `Promise`). Let infrastructure or
