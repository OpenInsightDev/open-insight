...
After confirmation, define a schema for the final grade. A named `Schema.Class` is a good default
for object-shaped domain results, but the encoded shape is determined by the task's grade contract.
Keep task inputs, diagnostics, and grade fields separate. Do not add a field solely because a
built-in metric expects a different shape; `Task.mapMetric` adapts the confirmed grade to the metric
instead.
...
An intermediate stage may use its own schema. The final stage's grade schema is the task grade;
there is no separate "end stage" constructor — the last `Task.stage` in the pipe is the final
stage.
...
- The final grade and every intermediate stage result use schemas matching their confirmed
  contracts.
...
- Graders return schema-compatible encoded values and do not hide infrastructure failures as
  passes.
