...
export const withGitRepo = Effect.fn("withGitRepo")(
  function* (repoURL: string, options: Options = {}) {
    const fs = yield* FileSystem.FileSystem;
    ...
    const repoPath =
      ...
      (yield* Effect.gen(function* () {
        ...
        return yield* Cache.cacheDir(path.join("git", key));
      }));
    ...
    yield* loadGitRepo(repoPath, repoURL, options).pipe(
      Effect.mapError((cause) => (cause instanceof TasksError ? cause : TasksError.source(cause))),
    );
