...
export const makeSnapshot = Effect.fn("Task.Load.makeHarborSnapshot")(function* (
  ...
  const environmentDir = path.join(path.resolve(taskDir), "environment");

  if (environment?.docker_image !== undefined) {
    return Snapshot.make({ image: environment.docker_image, context: environmentDir });
  }

  return yield* Snapshot.fromContainerfile({
    filePath: path.join(environmentDir, "Dockerfile"),
    context: environmentDir,
  });
});
...
export const makeHarborTask = <T extends HarborTask>(
  taskDir: string,
  TaskClass: HarborTaskClass<T>,
): Effect.Effect<T, TaskError, FileSystem.FileSystem | Path.Path> =>
  Effect.gen(function* () {
    ...
    if (config.environment?.os === "windows") {
      return yield* Effect.fail(
        TaskError.load(new Error("Windows Harbor tasks are not supported")),
      );
    }
