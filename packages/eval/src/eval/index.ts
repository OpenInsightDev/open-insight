...
type EventStream<T extends Task.AnyTask = Task.AnyTask, R = never> = Stream.Stream<
  Event.EvalEvent,
  Event.EvalErrorEvent | Event.BenchResult<Task.GradeTypeOf<T>>,
  R
>;
...
export const make =
  ...
  (bench: Bench.Bench<T>) =>
    makeStream<T>(bench, Config.make(configOptions));
...
export const stream = <T extends Task.AnyTask, R>(
  ...
  stream.pipe(
    Stream.catchTag("BenchResult", () => Stream.empty),
...
export const result = <T extends Task.AnyTask, R>(
  ...
  stream.pipe(
    ...
    Effect.catchTag("BenchResult", (result) => Effect.succeed(result)),
