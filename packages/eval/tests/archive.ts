...
async function* loadTasks(repoPath: string) {
  ...
  for (const entry of entries) {
    ...
    yield* makeTask({
      ...
      Task.trajMetric(
        TrajMetric.Tool.makeExec(({ toolTurns }) => {
          return TrajMetric.Tool.successRate(toolTurns["bash"]);
        }),
        {
          name: "Bash call success rate",
          description: "The success rate of bash calls in the task trajectory.",
          chart: (successRate) => [
            Chart.Pie.make({ legend: "Success", value: successRate }),
            Chart.Pie.make({ legend: "Fail", value: 1 - successRate }),
          ],
        },
      ),
      Task.trajMetric(
        TrajMetric.Tool.makeExec(({ toolTurns }) => {
          return TrajMetric.Tool.successCount(toolTurns["bash"]);
        }),
        {
          name: "Total bash calls",
          description: "The total number of bash calls in the task trajectory.",
        },
      ),
    );
  }
...
export const makeBench = Effect.fn("verilog-eval/makeBench")(function* () {
  const repoPath = yield* Tasks.withGithub("NVlabs/verilog-eval", {
    branch: "main",
    commit: "c498220d0a52248f8e3fdffe279075215bde2da6",
  });

  return yield* Bench.make("verilog-eval", pipe(repoPath, loadTasks, Tasks.fromAsyncIter)).pipe(
    Bench.mapMetric(({ simPass }) => ({ pass: simPass }), BenchMetric.avgPassAtK(1), {
      name: "Average pass@1",
      description: "Mean pass@1 estimate across evaluated tasks.",
      chart: (result) => [
        Chart.Pie.make({ legend: "Pass", value: result }),
        Chart.Pie.make({ legend: "Fail", value: 1 - result }),
      ],
    }),
  );
});
